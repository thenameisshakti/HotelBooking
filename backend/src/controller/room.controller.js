...
const createRoom = asynHandler( async (req,res) => {
    const hotelId = req.params.hotelid
    const {title, price, maxPeople , roomNumbers,...others}  = req.body 

    let  RoomNo
    if(roomNumbers){
       RoomNo  = typeof roomNumbers === "string" ? JSON.parse(roomNumbers) : roomNumbers
    }
    console.log(RoomNo)
    ...
    if(existedroom){
        throw new ApiError(404, "already exised room with same title or same room number")
    }
...
const updateRoom = asynHandler( async (req,res) => {
    ...
    if(update.roomNumbers){
        newRoom  = typeof update.roomNumbers === "string" ? JSON.parse(update.roomNumbers) : update.roomNumbers
...
const updateavailability = asynHandler(async (req,res) => {
    ...
    return res
    .status(200)
    .json( new ApiResponse(200,availabilityupdate , "unavailability update successfully"))

})

const deleteRoom = asynHandler( async (req, res ) => {
    const rid = req.params.roomNumbersid
    console.log(rid)
