...
function Book({ setOpen, hotelId, days }) {
  ...
  const { data, loading, error } = useFetch(
    `/api/v1/hotels/room/${hotelId}?startDate=${date[0].startDate}&endDate=${date[0].endDate}`,
  );
  console.log(data);
    ...
    try {
      if (checked) {
        ...
        setSelectedRoom((prev) =>
          prev.includes(roomNumberId) ? prev : [...prev, roomNumberId],
        );
  ...
  const handleCross = async () => {
    try {
      if (Object.keys(locks).length > 0) {
        await Promise.all(
          Object.values(locks).map((lockId) =>
  ...
  const checkoutHandler = async (amount) => {
    ...
    const {data} = await api.post("/api/v1/pay/payment", { amount,lockIds: Object.values(locks)});
    console.log(data)
    const options = {
      key: unique.data.key,
            ...
            >
              ...
              <div className="mb-4">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Choose available room numbers below
                </p>
              </div>
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 w-full sm:w-auto">
                  {item.roomNumbers.map((rno) => (
                    <label
                      ...
                      shadow-sm cursor-pointer transition select-none hover:bg-blue-50 hover:border-blue-400
                    `}
                    >
